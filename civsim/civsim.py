...
from third.rpgtools.main import generate_menu, roll
...
class Civilisation:
    def __init__(self, name, symbol, origin, ancestor=None, control=False):
        ...
        self.dead = False
        # Internal priorities
        self.priorities = [item for priority in CIV_PRIORITIES for item in (
    ...
    def update(self):
        ...
        self.power += self.base_power * \
            (0.8+(roll('6d10-6d10')/100)) + self.base_power*self.techlevel*3
        self.instability += randint(1,
                                    ceil(self.base_power/10)+1) - roll('3d6-3d6')
    ...
    def collapse(self, decimate=False):
        ...
        self.instability += roll(
            f'{ceil(self.base_power/10)}d6')
        ...
        if not decimate and lost_territory:  # Natural disasters do not create new states
            n = randint(1, ceil(self.base_power / 10) + 1)
            successor_states = sample(
                lost_territory, min(len(lost_territory), n))
            new_civs = []
    ...
    def execute(self):
        ...
        for n in range(curr_actions):
            if self.controlled_by_player:
                ...
                curr_option = CIV_PRIORITIES[generate_menu(
                    CIV_PRIORITIES)]
            else:
...
def display(world, year=None):
    ...
    for y in range(HEIGHT):
        for x in range(WIDTH):
            if CIV[y][x]:
                print(colored(CIV[y][x], 'red'), end='')
            else:
                print(colored(world[y][x], colour(world[y][x])), end='')
        print('')
